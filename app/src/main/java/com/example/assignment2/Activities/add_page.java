    ...
    private void addButton(){
        findViewById(R.id.btn_add).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                //Add to ROOM database(LocalDb)
                PhonebookDb.getInstance(getApplicationContext()).contactDao().insert(contact);

                //private void sendLocalDBtoServerSide() {
                    try {
                        //Contact c = new Contact(20, "kakeru", "010101", "urgex46@gmail.com", "12/12/1212");
                        RetrofitServices.getInstance().AddAContact(contact);

                        Toast.makeText(add_page.this, "Server-Side DB Updated", Toast.LENGTH_SHORT).show();
                    }catch(Exception ex)
                    {
                        Toast.makeText(add_page.this, "ERROR!! with Updating DB", Toast.LENGTH_SHORT).show();
                    }
                //}
